...
class BinaryWriter {
  public:
    ...
    void writeString16(const ::string &str) {
        ...
        if (len > 0) {
            ...
            if (bytes_written != len) {
                throw std::system_error(errno, std::generic_category(),
                                        "BinaryWriter failed to write string data");
            }
        }
    }

    // writes an array of PODs (like our std::vector<uint8_t> compressed bytes)
    template <typename T> void writeBuffer(const T *data, size_t count) {
        if (count == 0)
            return;
        ...
        if (bytes_written < 0 || static_cast<size_t>(bytes_written) != total_bytes) {
            throw std::system_error(errno, std::generic_category(),
                                    "BinaryWriter failed to write buffer");
        }
    }
    ...
    off_t currentOffset() const {
        ...
        if (offset == (off_t)-1) {
            throw std::system_error(errno, std::generic_category(),
                                    "BinaryWriter failed to get offset");
        }
        return offset;
    }
