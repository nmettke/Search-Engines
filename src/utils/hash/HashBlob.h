...
inline uint64_t HashBlobHashString(const char *key) {
    uint64_t h = 0;
    while (*key) {
        h = h * 37 + static_cast<unsigned char>(*key);
        ++key;
    }
    return h;
}

inline bool HashBlobCompareEqual(const char *a, const char *b) { return strcmp(a, b) == 0; }
...
class HashBlob {
  ...
  public:
    ...
    const SerialTuple *Find(const char *key) const {
        ...
        uint64_t hashValue = HashBlobHashString(key);
        ...
        while (true) {
            ...
            if (currentTuple->HashValue == hashValue &&
                HashBlobCompareEqual(currentTuple->Key, key)) {
                return currentTuple;
            }
    ...
    static size_t BytesRequired(const Hash *hashTable) {
        ...
        size_t totalBytes = 0;
        totalBytes += sizeof(HashBlob);
        totalBytes += hashTable->numberOfBuckets * sizeof(size_t);
...
class HashFile {
  ...
  public:
    ...
    HashFile(const char *filename) : blob(nullptr), mappedSize(0) {
    ...
    HashFile(const char *filename, const Hash *hashtable) : blob(nullptr), mappedSize(0) {
    ...
    ~HashFile() {
        if (blob != nullptr && mappedSize != 0) {
            munmap(blob, mappedSize);
        }
    }
};
